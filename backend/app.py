...
AVAILABLE_CARFAX_FILE = os.path.join(UPLOAD_FOLDER, 'available_carfax.csv')
...
def admin_panel():
    ...
    return render_template_string(ADMIN_TEMPLATE, last_update=last_update)
...
def trigger_update():
    try:
        ...
        return jsonify({"status": "Update triggered", "output": result.stdout})
...
def admin_carfax():
    ...
    # Get latest inventory and carfax file list
    ...
    return render_template_string(CARFAX_TEMPLATE, cars=cars)
...
<body>
  ...
  <table>
    ...
    <tr>
      <td>{{ car['vin'] }}</td>
      <td>{% if car['carfax_url'] %}<a href="{{ car['carfax_url'] }}" target="_blank">View Carfax</a>{% else %}No Carfax{% endif %}</td>
      <td>
